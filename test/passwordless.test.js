...
describe('Passwordless', () => {
  ...
  it('Should notify user about PassKey policy, but allow login without PassKey during grace period', async () => {
    ...
    // Assert
    expect(api.prompt.render).toHaveBeenCalledWith(event.secrets.NOTIFY_FORM_ID, {
      vars: {
  ...
  it('Should deny login without PassKey after grace period', async () => {
    // Prepare
    event.stats.logins_count = parseInt(event.secrets.MAX_LOGINS_WITHOUT_PASSKEY) + 1;
    // No PassKey used
    event.authentication.methods = [];
    ...
    // Assert
    expect(api.prompt.render).not.toHaveBeenCalledWith(event.secrets.NOTIFY_FORM_ID, {
      vars: {
        logins_left: logins_left,
      }
    })
    expect(api.prompt.render).toHaveBeenCalledWith(event.secrets.ENFORCE_FORM_ID)
  ...
  it('Should not deny login after grace period if a PassKey was used', async () => {
    ...
    expect(api.prompt.render).not.toHaveBeenCalledWith(event.secrets.ENFORCE_FORM_ID)
    expect(api.access.deny).not.toHaveBeenCalled()
  })
